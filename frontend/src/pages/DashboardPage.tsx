...
export const DashboardPage: React.FC = () => {
  const { canManageCustomers, canManageStock, canManageChallans } = useAuth();
  ...
  return (
    <div className="space-y-6 pb-12">
      ...
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-4 border-b border-zinc-200 dark:border-zinc-800 pb-5">
        <div>
          <div className="flex items-center gap-2">
            ...
            <span className="px-2.5 py-0.5 rounded-full text-[10px] font-mono uppercase bg-orange-100 dark:bg-orange-500/20 text-orange-700 dark:text-orange-400 border border-orange-200 dark:border-orange-500/30 font-semibold">
              Shift Active
            </span>
          </div>
        ...
        <div className="flex items-center gap-2 flex-wrap">
          {canManageChallans && (
            <button
              ...
              className="flex items-center gap-2 px-3.5 py-2 rounded-xl bg-orange-500 hover:bg-orange-600 text-white font-bold text-xs uppercase tracking-wider transition-all shadow-sm"
            >
          ...
          {canManageCustomers && (
            <button
              ...
              className="flex items-center gap-2 px-3.5 py-2 rounded-xl bg-white dark:bg-zinc-800 hover:bg-zinc-100 dark:hover:bg-zinc-700 border border-zinc-200 dark:border-zinc-700 text-zinc-800 dark:text-zinc-200 text-xs font-semibold uppercase tracking-wider transition-all shadow-sm"
            >
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
        ...
        <div className="p-5 rounded-2xl bg-white dark:bg-zinc-900 border border-zinc-200 dark:border-zinc-800 shadow-sm">
          ...
          <div className="mt-3">
            ...
            <div className="text-[11px] text-zinc-400 dark:text-zinc-500 mt-1 font-mono">
        ...
        <div className="p-5 rounded-2xl bg-white dark:bg-zinc-900 border border-red-200 dark:border-red-500/30 relative overflow-hidden shadow-sm">
          ...
          <div className="mt-3">
            {loadingLowStock ? (
              ...
              <div className="text-2xl font-bold font-mono text-red-600 dark:text-red-400 tracking-tight flex items-baseline gap-2">
                <span>{lowStockCount}</span>
                <span className="text-xs text-red-500/80 font-normal">items under threshold</span>
              </div>
            )}
            <div className="text-[11px] text-red-600/70 dark:text-red-300/70 mt-1 font-mono">
        ...
        <div className="p-5 rounded-2xl bg-white dark:bg-zinc-900 border border-zinc-200 dark:border-zinc-800 shadow-sm">
          ...
          <div className="mt-3">
            {loadingCustomers ? (
              ...
              <div className="text-2xl font-bold font-mono text-zinc-900 dark:text-white tracking-tight flex items-baseline gap-2">
          ...
          <div className="mt-3">
            {loadingChallans ? (
              ...
              <div className="text-2xl font-bold font-mono text-zinc-900 dark:text-white tracking-tight flex items-baseline gap-2">
                ...
                {draftChallans > 0 && (
                  <span className="text-xs text-orange-600 dark:text-orange-400 font-mono font-normal">
      ...
      {lowStockData && lowStockData.data.length > 0 && (
        <div className="p-5 rounded-2xl border border-red-200 dark:border-red-500/30 bg-red-50/60 dark:bg-red-950/20 shadow-sm">
          <div className="flex items-center justify-between mb-3">
            <div className="flex items-center gap-2">
              <AlertTriangle className="w-4 h-4 text-red-500" />
              <h3 className="text-xs font-mono uppercase tracking-wider font-bold text-red-700 dark:text-red-300">
                Critical Inventory Shortage Watchlist
              </h3>
            </div>
            <button
              onClick={() => navigate('/products?lowStock=true')}
              className="text-xs font-mono text-orange-600 dark:text-orange-400 hover:underline flex items-center gap-1 font-semibold"
            >
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 gap-3">
            {lowStockData.data.slice(0, 3).map((item) => (
              <div
                key={item.id}
                className="p-3 rounded-xl bg-white dark:bg-zinc-900/90 border border-red-200 dark:border-red-500/20 flex items-center justify-between shadow-sm"
              >
                <div>
                  <div className="text-xs font-bold text-zinc-900 dark:text-white font-mono">{item.sku}</div>
                  <div className="text-[11px] text-zinc-600 dark:text-zinc-300 truncate max-w-[170px]">
                    {item.name}
                  </div>
                </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
        ...
        <div className="rounded-2xl bg-white dark:bg-zinc-900 border border-zinc-200 dark:border-zinc-800 p-5 shadow-sm">
          <div className="flex items-center justify-between mb-4">
            <div className="flex items-center gap-2">
              <FileSpreadsheet className="w-4 h-4 text-orange-500" />
              <h2 className="text-xs font-mono uppercase tracking-wider font-bold text-zinc-900 dark:text-white">
                Recent Sales Challans
              </h2>
            </div>
            <button
              onClick={() => navigate('/challans')}
              className="text-xs font-mono text-zinc-500 dark:text-zinc-400 hover:text-orange-600 dark:hover:text-orange-400 transition-colors flex items-center gap-1 font-medium"
            >
              <span>View All</span>
              <ArrowUpRight className="w-3.5 h-3.5" />
            </button>
            ...
            {loadingChallans ? (
              ...
              challansData?.data.map((c) => (
                <div
                  ...
                  className="py-3 flex items-center justify-between hover:bg-zinc-50 dark:hover:bg-zinc-800/50 px-2.5 rounded-xl cursor-pointer transition-colors"
                >
                  <div>
                    <div className="flex items-center gap-2">
                    ...
                    <div className="text-xs text-zinc-600 dark:text-zinc-300 mt-0.5 truncate max-w-xs">
        ...
        <div className="rounded-2xl bg-white dark:bg-zinc-900 border border-zinc-200 dark:border-zinc-800 p-5 shadow-sm">
          <div className="flex items-center justify-between mb-4">
            <div className="flex items-center gap-2">
              <Warehouse className="w-4 h-4 text-zinc-500 dark:text-zinc-400" />
              <h2 className="text-xs font-mono uppercase tracking-wider font-bold text-zinc-900 dark:text-white">
          ...
          <div className="divide-y divide-zinc-100 dark:divide-zinc-800">
            {loadingMovements ? (
              ...
              movementsData?.data.map((m) => (
                <div key={m.id} className="py-3 flex items-center justify-between px-2.5 rounded-xl hover:bg-zinc-50 dark:hover:bg-zinc-800/40 transition-colors">
                  <div>
                    <div className="flex items-center gap-2">
                      <span
                    ...
                    <div className="text-[11px] text-zinc-500 dark:text-zinc-400 mt-0.5 truncate max-w-[220px]">
                      {m.reason}
                    </div>
                  </div>
                  <div className="text-right">
                    ...
                    <div className="text-[10px] font-mono text-zinc-400">
                      By {m.createdBy?.name?.split(' ')[0] || 'Staff'}
                    </div>
                  </div>
                </div>
              ))
