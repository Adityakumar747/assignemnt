    ...
    <div className="min-h-screen w-screen flex bg-white text-zinc-900 font-sans antialiased overflow-hidden">
      ...
      <div className="w-full lg:w-[48%] xl:w-[45%] flex flex-col justify-between p-6 sm:p-10 lg:p-12 xl:p-16 z-10 bg-white overflow-y-auto">
        ...
        <div className="my-auto py-8 max-w-md w-full mx-auto">
          ...
          <form onSubmit={handleSubmit} className="space-y-4">
            ...
            <div>
              ...
              <div className="relative">
                ...
                <input
                  ...
                  className="w-full pl-10 pr-10 py-2.5 bg-white border border-zinc-200 rounded-xl text-sm text-zinc-900 placeholder-zinc-400 focus:outline-none focus:border-orange-500 focus:ring-2 focus:ring-orange-500/15 transition-all shadow-sm font-sans"
                />
                <button
                  type="button"
            ...
            <div className="flex items-center justify-between text-xs pt-1">
              ...
              <a
                ...
                onClick={(e) => {
                  e.preventDefault();
                  alert('Please contact your system administrator to reset internal access credentials.');
                }}
            ...
            <button
              ...
              className="w-full mt-2 py-3 px-4 bg-zinc-900 hover:bg-black text-white font-semibold rounded-xl text-sm transition-all duration-150 flex items-center justify-center gap-2 shadow-md hover:shadow-lg disabled:opacity-50 cursor-pointer"
            >
              {isSubmitting ? (
          ...
          {/* Discreet Sample Credentials Guide (Clean, non-intrusive) */}
          <div className="mt-8 pt-4 border-t border-zinc-100">
            <div className="flex items-center justify-between text-xs text-zinc-500">
              <button
                type="button"
                onClick={() => setShowCredentialsHint(!showCredentialsHint)}
                className="inline-flex items-center gap-1.5 text-zinc-500 hover:text-orange-600 transition-colors font-medium text-xs cursor-pointer"
              >
                <HelpCircle className="w-3.5 h-3.5" />
                <span>{showCredentialsHint ? 'Hide test accounts' : 'Need test credentials? Click here'}</span>
              </button>
              <span className="text-[11px] text-zinc-400">4 Roles configured</span>
            </div>
            ...
            {showCredentialsHint && (
              <div className="mt-3 p-3.5 rounded-xl bg-zinc-50 border border-zinc-200 text-xs space-y-2 animate-fadeIn">
                ...
                <div className="grid grid-cols-2 gap-1.5">
                  <button
                    type="button"
                    onClick={() => fillSampleEmail('admin@ops.local')}
                    className="text-left px-2.5 py-1.5 rounded-lg border border-zinc-200 bg-white hover:border-orange-400 hover:bg-orange-50/30 transition-all cursor-pointer"
                  >
                    <div className="font-semibold text-xs text-zinc-800">Admin Account</div>
                    <div className="text-[10px] text-zinc-500 font-mono">admin@ops.local</div>
                  </button>

                  <button
                    type="button"
                    onClick={() => fillSampleEmail('sales@ops.local')}
                    className="text-left px-2.5 py-1.5 rounded-lg border border-zinc-200 bg-white hover:border-orange-400 hover:bg-orange-50/30 transition-all cursor-pointer"
                  >
                    <div className="font-semibold text-xs text-zinc-800">Sales Account</div>
                    <div className="text-[10px] text-zinc-500 font-mono">sales@ops.local</div>
                  </button>

                  <button
                    type="button"
                    onClick={() => fillSampleEmail('warehouse@ops.local')}
                    className="text-left px-2.5 py-1.5 rounded-lg border border-zinc-200 bg-white hover:border-orange-400 hover:bg-orange-50/30 transition-all cursor-pointer"
                  >
                    <div className="font-semibold text-xs text-zinc-800">Warehouse Account</div>
                    <div className="text-[10px] text-zinc-500 font-mono">warehouse@ops.local</div>
                  </button>

                  <button
                    type="button"
                    onClick={() => fillSampleEmail('accounts@ops.local')}
                    className="text-left px-2.5 py-1.5 rounded-lg border border-zinc-200 bg-white hover:border-orange-400 hover:bg-orange-50/30 transition-all cursor-pointer"
                  >
                    <div className="font-semibold text-xs text-zinc-800">Accounts Account</div>
        ...
        <div className="pt-4 border-t border-zinc-100 flex flex-col sm:flex-row items-center justify-between text-xs text-zinc-500 gap-3">
          <div className="flex items-center gap-4">
            ...
            <a
              ...
              onClick={(e) => {
                e.preventDefault();
                alert('Support: ops-support@enterprise.local | Extension #4102');
              }}
      ...
      <div className="hidden lg:block lg:w-[52%] xl:w-[55%] relative overflow-hidden bg-zinc-950">
        ...
        {/* Organic Curved Wave Divider on the left */}
